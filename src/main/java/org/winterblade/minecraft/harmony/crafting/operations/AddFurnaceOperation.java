...
public class AddFurnaceOperation extends BaseAddOperation {
    ...
    public void Init() throws ItemMissingException {
        ...
        if(with.getItemStack() == null) throw new RuntimeException("Unable to find requested input item.");
        if(with.hasNbt()) {
    ...
    @Override
    public void Apply() {
        System.out.println("Adding furnace recipe for " + output.getItemStack().getUnlocalizedName());
        float curXp = FurnaceRecipes.instance().getSmeltingExperience(output.getItemStack());
        if(curXp != 0.0F && curXp != experience) {
            System.out.println(output.getItemStack().getUnlocalizedName()
